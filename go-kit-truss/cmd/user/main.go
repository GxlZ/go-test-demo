...
import (
	...
	"go-test-demo/go-kit-truss/svc/client/cli/handlers"
	grpcclient "go-test-demo/go-kit-truss/svc/client/grpc"
	httpclient "go-test-demo/go-kit-truss/svc/client/http"
)
...
func submain() int {
	...
	fsGetUsernameV1 := flag.NewFlagSet("getusernamev1", flag.ExitOnError)

	var (
		flagIdGetUsernameV1 = fsGetUsernameV1.Int64("id", 0, "")
	)
	...
	case "getusernamev1":
		fsGetUsernameV1.Parse(flag.Args()[1:])

		IdGetUsernameV1 := *flagIdGetUsernameV1

		request, err := handlers.GetUsernameV1(IdGetUsernameV1)
		if err != nil {
			fmt.Fprintf(os.Stderr, "Error calling handlers.GetUsernameV1: %v\n", err)
			return 1
		}

		v, err := service.GetUsernameV1(ctx, request)
		if err != nil {
			fmt.Fprintf(os.Stderr, "Error calling service.GetUsernameV1: %v\n", err)
			return 1
		}
		fmt.Println("Client Requested with:")
		fmt.Println(IdGetUsernameV1)
